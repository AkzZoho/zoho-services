...
> It is **not** a description of the `.ds` sample files. Per the project
> rules in `LEARNING.md`, the `.ds` samples are treated as ground-truth input
> data and are intentionally **not** inspected here.
...
│  • samples/*.ds                  — reference Creator exports     │
...
[`docs/DELUGE_REFERENCE.md`](./DELUGE_REFERENCE.md). Any time a new
...
Per `docs/LEARNING.md` §1 (Rules 1, 5, 6), the following are out of scope:

- The **internal structure of the `.ds` sample files**. Those are input
  data; documenting them is governed by the sample-pattern catalog in
  `LEARNING.md §4`.
- Any **change suggestions** that are not already in code. Suggestions
  land in `LEARNING.md §5`, not here.
