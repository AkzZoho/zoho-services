...
> Basic / Intermediate / Advanced), plus observations distilled from the
> `.ds` exports we have analysed (the corpus itself is no longer kept in
> the repo — see [`project-layout.md`](./project-layout.md)).
>
...
## 5. Blueprint transition hooks

Blueprint scripts have the same Deluge syntax as workflow scripts but fire at transition points:

```deluge
// ── Before Transition ──────────────────────────────────────────────────────
// Runs before the stage changes. Can prevent transition with cancel submit.
rec = Form_Name[ID == input.ID];
if(rec.Notes == null)
{
    alert "Notes are required before this transition.";
    cancel submit;
}
...
rec = Form_Name[ID == input.ID];
rec.Status = "New Stage Name";
rec.Last_Transitioned_By = zoho.loginuser;
sendmail [ from: zoho.adminuserid to: rec.Email subject: "Stage Updated" message: "..." ]
insert into Activity_Log [ Parent = input.ID Action = "Transition Name" Time = zoho.currenttime ]
```

Inside blueprint scripts:
- `input.ID` = the record ID being transitioned
- `zoho.loginuser` = the user who clicked the transition button
- `input.*` = fields of the record being transitioned

---

## 5a. Batch Workflow script context

Batch workflows iterate records. `input.*` = **each individual record** as it is processed:

```deluge
// Batch Workflow — per-record script
// input.* is the CURRENT record being iterated

if(input.Status == "Pending" && input.Due_Date < zoho.currentdate)
{
    input.Status = "Overdue";  // Direct field update — no insert needed
    sendmail [ from: zoho.adminuserid to: input.Email subject: "..." message: "..." ]
}

insert into Batch_Log [ Record_ID = input.ID Processed_On = zoho.currenttime ]
```
