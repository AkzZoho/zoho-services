...
function parseFunctions(body, result) {
  ...
  for (const lang of extractTopLevelBlocks(body)) {
    ...
    while ((m = sigRe.exec(lang.inner)) !== null) {
      ...
      // Preserve the full function source so downstream tools (find-usages,
      // refactoring suggestions) can search inside the Deluge body and report
      // accurate file/line locations to the consultant.
      const sourceCode = sliced
        ? `${returnType} ${fullName}(${paramStr}) {${sliced.inner}}`
        : '';
